...
def verificar_cartelas_batidas(cartelas, numeros_sorteados):
    """Verifica se alguma cartela tem todos os números sorteados."""
    cartelas_batidas = []
...
def sorteio_bingo(cartelas):
    """Realiza o sorteio dos números e verifica as cartelas batidas."""
    numeros_disponiveis = list(range(1, 76))
    numeros_sorteados = random.sample(numeros_disponiveis, 25)  # Mantém o sorteio inicial de 25 números
...
def gerar_cartelas():
    """Gera cartelas de bingo e retorna um dicionário com as cartelas criadas."""
    ...
    while len(cartelas) < num:
        ...
        for i in range(5):
            while len(colunas[i]) < 5:
                ...
                if numero not in colunas[i]:
                    colunas[i].append(numero)

        cartela = [sorted(coluna) for coluna in colunas]

        # Garante que o número correspondente da cartela seja único
        while True:
...
def salvar_cartelas(cartelas, inicializar=False):
    """Salva as cartelas geradas em um arquivo de texto."""
    modo = "w" if inicializar else "a"
...
def imprimir_cartela(cartelas, numero_cartela):
    """Imprime a cartela especificada no formato de tabela."""
...
def ler_cartelas():
    """Lê as cartelas salvas do arquivo e organiza em um dicionário."""
    try:
        ...
        for linha in linhas:
            partes = linha.split(": ")
            numero_cartela = int(partes[0])
            cartela_str = partes[1].strip()
            cartela = eval(cartela_str)
        ...
        print("O arquivo 'cartelas.txt' não foi encontrado.")
        return None

def menu_bingo():
    """Menu principal para navegação entre as funcionalidades do Bingo."""
